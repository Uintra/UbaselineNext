...
class Grid {
    ...
    container(options)
    {
        const size = parseFloat(options.value);
        if (size) {
            return `${size}px`;
        }
    ...
    calcSpanWidth(options)
    {
        ...
        let count = parsedOptions.columns;
        let calculated = this.calc(options);
        let width = count * (calculated.column + calculated.gutter * 2) - calculated.gutter;

        if (parsedOptions.wide)
        {
            width += calculated.gutter;
        }

        if (parsedOptions.wider)
        {
            width += (calculated.gutter * 2);
        }
    ...
    parseOptions(options)
    {
        ...
        let columns = parseFloat(parts[0]);
